...
export function getSmartRecommendations(cartDetails) {
  ...
  const flooringBrands = Object.values(cartDetails || {})
    .filter(item => item.pricePerSqFt)
    .map(item => item.brand)
    .filter(Boolean);
  ...
  const recommendations = products
    .filter(product => {
      if (product.type !== 'Accessory') return false;
      if (accessoriesInCart.includes(product.id)) return false;

      const recommendedFor = product.coverage?.recommendedFor || product.recommendedFor;
      if (!recommendedFor?.some(type => flooringTypes.includes(type))) return false;
      if (product.brand && !flooringBrands.includes(product.brand)) return false;

      return Boolean(product.coverage || product.alwaysRecommend);
    })
    .map(accessory => {
      const recommendedQuantity = accessory.coverage
        ? calculateAccessoryQuantity(totalFlooringSqFt, accessory)
        : Math.max(1, accessory.recommendedQuantity || 1);

      return {
        ...accessory,
        recommendedQuantity,
        coverageDescription: accessory.coverage
          ? `Covers approximately ${accessory.coverage.sqFtPerUnit} sq ft per ${
              accessory.subType === 'Adhesive' ? 'gallon' : 'unit'
            }`
          : null
      };
    });
