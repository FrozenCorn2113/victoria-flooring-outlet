...
7. If it's appropriate to follow up, politely ask for their email and phone number so Ty can reach out
...
export function buildContext({ pageUrl, productViewed, cartContents }) {
  ...
  if (cartContents && Object.keys(cartContents).length > 0) {
    ...
    Object.values(cartContents).forEach(item => {
      context += `\n- ${item.name}: ${item.quantity} ${item.pricePerSqFt ? 'sq ft' : 'units'}`;
    });
  }
