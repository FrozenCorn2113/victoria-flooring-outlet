  ...
  const TimeUnit = ({ value, label }) => (
    <div className="flex flex-col items-center">
      <div className="bg-vfo-charcoal px-3 py-2 md:px-3.5 md:py-2.5 rounded shadow-md min-w-[48px] md:min-w-[56px]">
        <span className="text-xl md:text-2xl font-semibold text-white tabular-nums">
          {String(value).padStart(2, '0')}
        </span>
      </div>
      <span className="text-[10px] md:text-xs uppercase tracking-wider text-vfo-charcoal mt-1.5 font-medium">
        {label}
      </span>
    </div>
  );
